...
def write_index(repo_root: Path) -> Path:
    ...
    lines = ["# Runs", "", "Generated by `python3 tools/gov.py report`. One row per run; "
             "open `runs/<id>/audit.md` for the record. 'attempted violations' counts "
             "denied calls that would have broken policy (writes out of scope, protected "
             "paths, disallowed commands); the audit also lists denials that were not "
             "violations, such as a disallowed search tool.", "",
