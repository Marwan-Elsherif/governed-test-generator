...
Feature: API GET /cart/{id}/total
  ...
  Scenario: returns 200 when the cart exists with items
    Given a cart exists with id "00000000-0000-4000-8000-000000000201" and subtotal "50.00"
    When I send a GET request to "/cart/00000000-0000-4000-8000-000000000201/total"
    Then the response status is 200
    And the response body has "$.subtotal" equal to "50.00"
    And the response body has "$.tax" equal to "10.50"
    And the response body has "$.total" equal to "60.50"

  @status-200 @ac-5
  Scenario: returns 200 when the cart is empty
    Given a cart exists with id "00000000-0000-4000-8000-000000000202"
